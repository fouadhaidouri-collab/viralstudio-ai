...
export async function createWithdrawalRequest({ user_id, amount, payment_method, payment_account }) {
  ...
  await run(
    ...
    [makeId("notif"), "admin", "New Withdrawal Request", `User ${user_id} requested $${amount} via ${payment_method}`]
  );
...
export async function updateWithdrawalRequest(id, updates) {
  ...
  if (!wd) return null;
  if (updates.status === "approved") {
    await run("UPDATE affiliate_accounts SET paid_balance = COALESCE(paid_balance, 0) + ? WHERE user_id = ?", [wd.amount, wd.user_id]);
    await run(
      "INSERT INTO notifications (id, user_id, title, message, created_at) VALUES (?, ?, ?, ?, datetime('now'))",
