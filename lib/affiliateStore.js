...
export async function updateWithdrawal(id, updates) {
  const old = await get("SELECT * FROM withdrawals WHERE id = ?", [id]);
  if (!old) return null;
  const fields = [];
  const values = [];
  for (const [key, val] of Object.entries(updates)) {
  ...
  if (updates.status === "completed") {
    ...
    if (wd) {
      ...
      const aff = await getAffiliateById(wd.affiliate_id);
      if (aff) {
        await run(
          "INSERT INTO notifications (id, user_id, title, message, created_at) VALUES (?, ?, ?, ?, datetime('now'))",
          [makeId("notif"), aff.user_id, "Withdrawal Approved", `Your $${wd.amount} withdrawal has been approved and processed.`]
        );
      }
    }
  } else if (updates.status === "rejected" && old.status === "pending") {
    await run("UPDATE affiliate_accounts SET available_balance = COALESCE(available_balance, 0) + ? WHERE id = ?", [old.amount, old.affiliate_id]);
    const aff = await getAffiliateById(old.affiliate_id);
    if (aff) {
      await run(
        "INSERT INTO notifications (id, user_id, title, message, created_at) VALUES (?, ?, ?, ?, datetime('now'))",
        [makeId("notif"), aff.user_id, "Withdrawal Rejected", `Your $${old.amount} withdrawal request has been rejected. The amount has been returned to your balance.`]
      );
    }
  }
