...
export async function getUserCredits(userId) {
  const row = await get("SELECT * FROM credits WHERE user_id = ?", [userId]);
  if (row) return { user_id: row.user_id, balance_credits: row.current_balance || 0, updated_at: row.updated_at };
...
export async function saveUserCredits(userId, balance) {
  ...
  if (existing) {
    await run("UPDATE credits SET current_balance = ?, total_purchased = ?, total_used = ?, updated_at = datetime('now') WHERE user_id = ?", [balance, existing.total_purchased || 0, existing.total_used || 0, userId]);
  } else {
    const id = `cred_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await run("INSERT INTO credits (id, user_id, current_balance, total_purchased, total_used, updated_at) VALUES (?, ?, ?, 0, 0, datetime('now'))", [id, userId, balance]);
  }
}
...
export async function addUserCredits(userId, amount, type = "purchase", metadata = {}) {
  ...
  const existing = await get("SELECT * FROM credits WHERE user_id = ?", [userId]);
  if (existing) {
    await run("UPDATE credits SET current_balance = ?, total_purchased = total_purchased + ?, updated_at = datetime('now') WHERE user_id = ?", [newBalance, amount, userId]);
  } else {
    const id = `cred_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await run("INSERT INTO credits (id, user_id, current_balance, total_purchased, total_used, updated_at) VALUES (?, ?, ?, ?, 0, datetime('now'))", [id, userId, newBalance, amount]);
  }
  const tx = {
...
export async function deductUserCredits(userId, amount, moduleId, endpointId, metadata = {}) {
  ...
  const existing = await get("SELECT * FROM credits WHERE user_id = ?", [userId]);
  if (existing) {
    await run("UPDATE credits SET current_balance = ?, total_used = total_used + ?, updated_at = datetime('now') WHERE user_id = ?", [newBalance, amount, userId]);
  } else {
    const id = `cred_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    await run("INSERT INTO credits (id, user_id, current_balance, total_purchased, total_used, updated_at) VALUES (?, ?, ?, 0, ?, datetime('now'))", [id, userId, newBalance, amount]);
  }
